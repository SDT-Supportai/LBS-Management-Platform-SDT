...
export interface AccessoryRequest {
  ...
  status: AccReqStatus
  // เบิกให้ Service แล้วเมื่อ / โดยใคร (0059) — ว่าง = ยังไม่เคยเบิกออกไปเลย
  // ⚠️ คนละเรื่องกับ status = 'issued' ที่หมายถึง "เบิกจากคลังคงเหลือเข้า Job แล้ว"
  // ตั้งแต่ 0067 เบิกได้ทีละบางส่วน ⇒ 2 ช่องนี้เป็น "รอบล่าสุดที่เบิก" ไม่ใช่ "เบิกครบแล้ว"
  //   เลิกใช้เป็นธง "จบแล้ว" — ตัวที่บอกว่าจบคือ accSettled() / qtyPendingIssue() ใน logic.ts
  issuedToServiceAt?: string
  issuedToServiceBy?: string
  /**
   * จำนวนที่เบิกออกไปให้ Service แล้วรวมทุกรอบ (0067) — สะสมขึ้นเรื่อย ๆ
   * ค่าว่าง/undefined = ข้อมูลก่อน 0067 (ตอน backfill ถือว่าเบิกครบทั้งบรรทัด)
   * ⚠️ **ไม่กระทบต้นทุน Job** — ต้นทุนตัดตาม qtyRequested − qtyTransferred เหมือนเดิม
   *    การเบิกคือย้ายของไปหน้างาน ไม่ใช่คืนของ · มีแต่การโอนคืนคลังที่ลดต้นทุน
   */
  qtyIssuedToService?: number
  /**
   * ตัดจำหน่ายของเหลือที่ Job (0068) — ของที่ไม่คุ้มจะโอนคืนคลัง (น็อต 3 ตัว · สายเหลือ 2 เมตร)
   * ⚠️ **ต้นทุนยังอยู่กับ Job** — ของถูกใช้/สูญไปกับงานนี้จริง ไม่ใช่คืนของ
   *    ต่างจาก qtyTransferred ที่ตัดต้นทุนออกเพราะของกลับไปให้ Job อื่นใช้ได้
   * ⚠️ **ไม่ลง stock_movements** — ของไม่เคยอยู่ในคลังกลาง (ถูกตัดออกตอนเบิกเข้า Job
   *    หรือซื้อตรงผ่าน PO) · ลงเป็นแถวในบัญชีเดินสะพัดจะทำให้รายงาน "ปริมาณออกจากคลัง"
   *    นับซ้ำ · หลักฐานอยู่ที่ Audit Log + 3 ช่องนี้ (ดูได้ในป๊อปอัปประวัติการเคลื่อนไหว)
   */
  qtyWrittenOff?: number
  writeOffReason?: string      // เหตุผลของรอบล่าสุด (บังคับกรอก)
  writtenOffAt?: string
  writtenOffBy?: string

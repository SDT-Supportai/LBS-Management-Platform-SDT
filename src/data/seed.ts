...
const ITEMS: Item[] = [
  ...
  { id: 'i-ct', code: 'ACC-CT-01', name: 'Current Transformer', itemType: 'accessory', uom: 'ชุด', stockableCentrally: true },
  { id: 'i-bracket', code: 'ACC-BRK-01', name: 'Mounting Bracket', itemType: 'accessory', uom: 'ชุด', stockableCentrally: true },
  { id: 'i-relay', code: 'ACC-RLY-01', name: 'Protection Relay', itemType: 'accessory', uom: 'ตัว', stockableCentrally: false },
  { id: 'i-cable', code: 'ACC-CBL-01', name: 'Control Cable 25m', itemType: 'accessory', uom: 'ม้วน', stockableCentrally: false },
]

function serials(prefix: string, from: number, count: number): string[] {
  return Array.from({ length: count }, (_, i) => `${prefix}-${String(from + i).padStart(3, '0')}`)
}
...
export function buildSeedDb(): DB {
  ...
  db = L.createProjectStock(db, sales, {
    ...
    serialNos: serials('LBS24', 1, 30), notes: 'ล็อตสั่งซื้อรอบที่ 1 (30 set)',
  })
  db = L.createProjectStock(db, sales, {
    stockNo: 'Project Stock No.2', itemId: 'i-lbs',
    serialNos: serials('LBS25', 1, 10), notes: 'ล็อตสั่งซื้อรอบที่ 2 (10 set)',
  })
  ...
  db = L.addAccessoryRequest(db, project, { jobId: job2, itemId: 'i-ct', qty: 5, source: 'central_stock' })
  db = L.addAccessoryRequest(db, project, { jobId: job2, itemId: 'i-relay', qty: 5, source: 'purchasing' })
  db = L.addAccessoryRequest(db, project, { jobId: job2, itemId: 'i-cable', qty: 3, source: 'purchasing' })
  ...
  db = L.addAccessoryRequest(db, project, { jobId: job3, itemId: 'i-bracket', qty: 2, source: 'central_stock' })
  ...
  db = L.addAccessoryRequest(db, project, { jobId: job4, itemId: 'i-ct', qty: 1, source: 'central_stock' })

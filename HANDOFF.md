...
> 🟠 **ค้างอยู่ (2026-09-18): ต้องรัน `0077` บน production ก่อน push รอบถัดไป**
> — เปิดสิทธิ์ `rpc_update_job_budget` จาก "Manage เท่านั้น" เป็น "Project เจ้าของงาน + Manage"
> ⇒ push frontend ก่อนรัน SQL = Project กดปุ่ม "แก้ไขงบประมาณ (ย้อนหลัง)" แล้วได้
> "แผนกของคุณไม่มีสิทธิ์ทำรายการนี้" จาก server
>
> ✅ 0070–0076 รันบน production แล้วทุกไฟล์ และ push ขึ้น `main` ครบ ·
> ล่าสุด **`bc244eb`** (แยกนัดติดตั้งออกจากกำหนดส่ง — frontend อย่างเดียว ไม่มี SQL)
>
...
lbs-platform/
  ...
  supabase/
    migrations/0001..0077    schema, RPC, seed, bug fixes, ฟีเจอร์ (ดูตารางหัวข้อ 5)
...
> DB ใหม่บนโปรเจกต์เปล่า: รัน **0001→0077** เรียงกันได้เลย (0004/0005 ผสานเข้า 0001/0002 ต้นทางแล้ว แต่ยังเก็บไฟล์แยกไว้เป็นประวัติ · 0012/0013 ถูก 0014 ยกเลิกแต่ต้องรันเรียงเพราะ 0014 อ้างถึงของที่มันสร้าง — ทุกไฟล์ idempotent รันซ้ำได้)

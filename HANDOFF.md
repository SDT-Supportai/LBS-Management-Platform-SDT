...
> DB ใหม่บนโปรเจกต์เปล่า: รัน **0001→0063** เรียงกันได้เลย (0004/0005 ผสานเข้า 0001/0002 ต้นทางแล้ว แต่ยังเก็บไฟล์แยกไว้เป็นประวัติ · 0012/0013 ถูก 0014 ยกเลิกแต่ต้องรันเรียงเพราะ 0014 อ้างถึงของที่มันสร้าง — ทุกไฟล์ idempotent รันซ้ำได้)
...
| `purchasing` | Purchasing | ออก PO / ยกเลิก PO (ยังไม่รับของ) / ตีกลับ PR / รับของ (partial ได้) / **แก้เลข PR ให้ตรงเอกสารจริง — เฉพาะใบที่ยังไม่ออก PO (0047)** · **Material Database (0061)** — เพิ่ม/แก้/ลบ Accessory + Export/Import Excel + ปรับยอดคลังคงเหลือ + Lot No. · **⚠️ ไม่รวมคลัง LBS / Project Stock** ซึ่งยังเป็นของ Division เท่านั้น |
...
**Material Database (0061)**: perm `material.manage` = `purchasing` + `admin` (เพิ่ม/แก้/ลบ Accessory + Import Excel) · perm `accessoryStock.manage` = `sales` + `purchasing` + `admin` (ปรับยอดคลังคงเหลือ + Lot No.) · **Export Excel เปิดให้ทุกแผนกที่ login มาตั้งแต่ต้น** (อ่านอย่างเดียว ไม่มี gate)

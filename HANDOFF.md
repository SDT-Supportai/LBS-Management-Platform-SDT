...
> ✅ **ไม่มี SQL / frontend ค้าง (2026-09-17)** — 0070–0076 รันบน production แล้วทุกไฟล์
> และ push ขึ้น `main` ครบ · ล่าสุด **`2875868`** (0075+0076 สถานะกำหนดส่ง + เอกสารแนบรายงวด)
>
...
lbs-platform/
  ...
  supabase/
    migrations/0001..0076    schema, RPC, seed, bug fixes, ฟีเจอร์ (ดูตารางหัวข้อ 5)
...
> DB ใหม่บนโปรเจกต์เปล่า: รัน **0001→0076** เรียงกันได้เลย (0004/0005 ผสานเข้า 0001/0002 ต้นทางแล้ว แต่ยังเก็บไฟล์แยกไว้เป็นประวัติ · 0012/0013 ถูก 0014 ยกเลิกแต่ต้องรันเรียงเพราะ 0014 อ้างถึงของที่มันสร้าง — ทุกไฟล์ idempotent รันซ้ำได้)
...
### 🟠 Migrations — ✅ 0070–0076 รันครบ (ยืนยัน 2026-09-15 → 09-17) · ⚠️ 0060–0069 ไม่มีบันทึกยืนยันในเอกสาร
...
> | 0070–0076 | ✅ รันครบ · ผู้ใช้ยืนยันรายไฟล์ตอน push แต่ละรอบ (0070 · 0071 · 0072 · 0073 · 0074 · 0075+0076) |
>
...
- 🔴 **"เลยกำหนด" ห้ามคำนวณจาก `daysLeft < 0` ที่หน้าใดหน้าหนึ่งเองเด็ดขาด — ต้องมาจาก `jobDelivery()` เท่านั้น** (0075)
  นี่คือบั๊กที่ผู้ใช้รายงานเอง: Dashboard ขึ้น **"เลยกำหนด" สีแดงกับงานที่ช่างกำลังติดตั้งอยู่หน้างาน**
  เพราะทุกหน้าเทียบ `requiredDate` กับวันนี้อย่างเดียว ไม่เคยดูว่าของออกจากคลังไปแล้วหรือยัง
  ⇒ ใบที่เดินอยู่จริงกับใบที่ไม่มีใครแตะเป็นสีเดียวกัน · พอทุกอย่างแดงหมด คนอ่านก็เลิกมองสีแดง = **ตัวเตือนตายทั้งระบบ**
  กติกาของโมเดลใหม่ (`DeliveryState` ใน `logic.ts`) — ลำดับตัดสินเรียงตาม "สิ่งที่ต้องลงมือก่อน" ไม่ใช่ความรุนแรงของวันที่:
  `closed` → `blocked` → `awaiting_close` → `in_field_late` / `in_field` → `overdue` → `due_soon` → `on_track` → `no_due`
  · **`overdue` (แดง) เกิดได้เฉพาะตอนของยังไม่ออกจากคลัง** · ของออกไปแล้วแต่เลยแผน = `in_field_late` (ส้ม)
  · `blocked` มาก่อน `in_field_late` เสมอ — เครื่องที่ติดตั้งไม่ได้คือของจริงที่ค้าง ส่วนวันที่ยังเลื่อนได้
  · `awaiting_close` มาก่อนเรื่องวันที่ — ทุกเครื่องได้ข้อสรุปแล้ว เหลือแค่กดปิด ไล่เรื่องวันตอนนี้ไม่ช่วยอะไร
  · ตัวนับ "ต้องเร่ง" ทุกหน้าใช้ `d.atRisk` (= `overdue` + `blocked`) **ห้ามนับ `daysLeft < 0` เอง**
  · ป้ายบนจอมาจาก `<DeliveryBadge d={...} />` ตัวเดียว ใช้ร่วมกัน 5 หน้า — เพิ่มหน้าใหม่ให้ใช้ตัวนี้ อย่าเขียนสีเอง
  ⚠️ **แกน "กำหนดส่ง" กับ "นัดติดตั้ง" เป็นคนละเรื่อง ห้ามยุบรวม**: `installStartDate/EndDate` = วันที่ทีมช่างจะออกไซต์
  (Service เลื่อนเองได้ผ่าน `logSiteVisit` outcome `rescheduled`) ส่วนกำหนดส่งเป็นข้อผูกพันกับลูกค้า
  **เลื่อนได้เฉพาะผ่าน `extendJobDue` พร้อมเหตุผล** · หน้า Service จึงเตือน (ไม่บล็อก) เมื่อวันนัดใหม่เลยกำหนดส่ง
  ⚠️ `jobs.required_date` **ห้ามเขียนทับเวลาขยายกำหนด** — เป็นกำหนดตามสัญญาที่ใช้เทียบว่าเลื่อนรวมกี่วัน
  ทุกจุดที่โชว์กำหนดใหม่ต้องโชว์ "เลื่อนจาก <วันเดิม>" กำกับ รวมถึงไฟล์ Excel และข้อความที่บอทตอบใน LINE

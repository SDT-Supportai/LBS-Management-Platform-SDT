...
| `0018_notify_add_units.sql` | **bug fix (2026-07-19)**: `rpc_add_units_to_stock` เดิมไม่มี app_notify → "รับ LBS เพิ่มเข้าคลังเดิม" + **Excel Import** ไม่แจ้งเข้า LINE เลย (มีแต่ตอนสร้างคลังใหม่) → เพิ่ม `stock_received` (dept project) · demo sync ที่ `logic.ts` addUnitsToStock |

> DB ใหม่บนโปรเจกต์เปล่า: รัน 0001→0018 เรียงกันได้เลย (0004/0005 ผสานเข้า 0001/0002 ต้นทางแล้ว แต่ยังเก็บไฟล์แยกไว้เป็นประวัติ · 0012/0013 ถูก 0014 ยกเลิกแต่ต้องรันเรียงเพราะ 0014 อ้างถึงของที่มันสร้าง — ทุกไฟล์ idempotent รันซ้ำได้)
